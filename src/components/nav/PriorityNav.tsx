...
type Props = { items: Item[]; moreLabel?: string };

export default function PriorityNav({ items, moreLabel = "Mehr…" }: Props) {
  ...
  const reflow = () => {
    ...
    const max = containerRef.current.clientWidth - 96; // deja espacio para CTA/WhatsApp
    let total = 0;
    const vis: Item[] = [];
    const over: Item[] = [];
    items.forEach((it) => {
  ...
  return (
    <div ref={containerRef} className="priority-nav flex items-center">
